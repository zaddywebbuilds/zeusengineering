      ...
      <div className={cx("grid grid-cols-1 gap-px bg-[var(--rule)]", cols)}>
        {facts.map((fact) => (
          <div
            ...
            className="reveal bg-graphite px-0 py-8 sm:px-7 sm:py-9"
          >
            <div className="flex items-baseline gap-1.5">
              ...
              {fact.unit && (
                <span className="numeral text-[clamp(1.25rem,2vw,1.75rem)] text-steel">
                  {fact.unit}
                </span>
              )}
            </div>
            ...
            {fact.note && (
              <p className="mt-3 max-w-[32ch] text-sm leading-relaxed text-steel-dim">
                {fact.note}
              </p>
            )}
          </div>
