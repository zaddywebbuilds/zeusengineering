...
export async function generateMetadata({
  ...
  return pageMeta({
    ...
    title: t("Careers"),
    description:
      t("ZEUS Engineering is a small engineering team in southern Vietnam working on power, thermal management and high-density compute infrastructure."),
    path: "/company/careers",
  });
}

export default async function CareersPage({ params }: LangPageProps) {
  const { lang: rawLang } = await params;
  const lang = toLocale(rawLang);
  const t = translator(lang);
    ...
    <>
      ...
      <Section>
        ...
        <div className="border border-dashed border-[var(--rule-strong)] p-10 lg:p-16">
          ...
          <Button lang={lang} href={`mailto:${company.email}?subject=General interest`} className="mt-9">
