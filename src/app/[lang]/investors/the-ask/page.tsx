...
export async function generateMetadata({
  ...
  return pageMeta({
    ...
    title: t("The Ask"),
    description:
      t("ZEUS Engineering is raising $2.69M for an 18–24 month runway to multiple live SSMDC nodes. Full use-of-funds breakdown."),
    path: "/investors/the-ask",
  });
}

export default async function TheAskPage({ params }: LangPageProps) {
  const { lang: rawLang } = await params;
  const lang = toLocale(rawLang);
  const t = translator(lang);
