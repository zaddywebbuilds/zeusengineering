...
export async function generateMetadata({
  ...
  return pageMeta({
    lang,
    title: t("Expansion Roadmap"),
    description:
      t("From ZEUS Engineering's operating prototype through the first SSMDC nodes to a distributed network."),
...
export default async function RoadmapPage({ params }: LangPageProps) {
  const { lang: rawLang } = await params;
  const lang = toLocale(rawLang);
  const t = translator(lang);
    ...
    <>
      ...
      <Section>
        ...
        <div className="reveal mt-10 flex flex-wrap items-center gap-4">
          <Button lang={lang} href="/investors/economics">{t("Node economics")}</Button>
