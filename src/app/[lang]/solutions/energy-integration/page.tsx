...
export async function generateMetadata({
  ...
  return pageMeta({
    lang,
    title: t("Energy Integration"),
    description:
      t("Grid supply and solar generation managed as one system, ZEUS Engineering's approach to reducing the operating cost of high-density compute in Vietnam."),
    path: "/solutions/energy-integration",
  });
}

export default async function EnergyIntegrationPage({ params }: LangPageProps) {
  const { lang: rawLang } = await params;
  const lang = toLocale(rawLang);
  const t = translator(lang);
