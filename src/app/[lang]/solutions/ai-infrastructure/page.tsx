...
export async function generateMetadata({
  ...
  return pageMeta({
    lang,
    title: t("AI Infrastructure"),
    description:
      t("The physical requirements of high-density AI compute, and how ZEUS Engineering's operating experience in Vietnam applies to them."),
    path: "/solutions/ai-infrastructure",
  });
}

export default async function AiInfrastructurePage({ params }: LangPageProps) {
  const { lang: rawLang } = await params;
  const lang = toLocale(rawLang);
  const t = translator(lang);
    ...
    <>
      ...
      <Section tone="linen">
        ...
        <div className="reveal mt-10 flex flex-wrap items-center gap-4">
          <Button lang={lang} href="/solutions/modular-data-centers">
            Modular data centers
          </Button>
          <Button lang={lang} href="/contact?intent=build" variant="secondary">
            Build with Zeus
          </Button>
        </div>
      </Section>
