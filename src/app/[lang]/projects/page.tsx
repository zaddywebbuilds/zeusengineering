...
export async function generateMetadata({
  ...
  return pageMeta({
    ...
    title: t("Projects"),
    description:
      t("ZEUS Engineering's operating infrastructure. Project 001: a reported 100 kW facility on a 300 m² site in the Vung Tau / Ba Ria region, Vietnam."),
    path: "/projects",
  });
}

export default async function ProjectsPage({ params }: LangPageProps) {
  const { lang: rawLang } = await params;
  const lang = toLocale(rawLang);
  const t = translator(lang);
        ...
        >
          <div>
            <div className="flex flex-wrap items-center gap-4">
              ...
              <StatusBadge status="zeus-reported" label={t("Operating")} />
            </div>
            ...
            <span className="mt-10 inline-flex items-center gap-3 text-sm">{t("Explore project")}<span
                aria-hidden
