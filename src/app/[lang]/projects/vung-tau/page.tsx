...
export async function generateMetadata({
  ...
  return pageMeta({
    ...
    description:
      t("ZEUS Engineering's operating site in Ba Ria, Vietnam: a reported 100 kW facility on 300 m² with 20 kW peak solar."),
...
export default async function ProjectPage({ params }: LangPageProps) {
  const { lang: rawLang } = await params;
  const lang = toLocale(rawLang);
  const t = translator(lang);
    ...
    <>
      ...
      <Section tone="linen">
        ...
        <div className="reveal mt-12 flex flex-wrap items-center gap-4">
          <Button lang={lang} href="/investors">{t("Investor relations")}</Button>
